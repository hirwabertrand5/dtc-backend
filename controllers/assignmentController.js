...
exports.autoAssignLinked = async (req, res) => {
  ...
  try {
    ...
    const crewFilter = { isArchived: false, status: 'Available', role: 'Driver' };
    ...
    const freeBuses = buses.filter(b => !busBusy.has(b._id.toString()))
      .filter(b => {
    ...
    let freeDrivers = drivers.filter(c => !crewBusy.has(c._id.toString()))
      .filter(c => {
    ...
    for (const drv of freeDrivers) {
      for (const bus of freeBuses) {
        const [existingCrewAssignments, existingBusAssignments] = await Promise.all([
          useTxn
            ? Assignment.find({ crewId: drv._id, status: { $in: ['Planned','Live'] }, ...overlapsQuery(start, end) }).session(session)
            : Assignment.find({ crewId: drv._id, status: { $in: ['Planned','Live'] }, ...overlapsQuery(start, end) }),
          useTxn
            ? Assignment.find({ busId:  bus._id, status: { $in: ['Planned','Live'] }, ...overlapsQuery(start, end) }).session(session)
            : Assignment.find({ busId:  bus._id, status: { $in: ['Planned','Live'] }, ...overlapsQuery(start, end) })
        ]);
        const conflicts = await evaluateConstraints({
          settings, crew: drv, bus, route,
          existingCrewAssignments, existingBusAssignments,
    ...
    if (needConductor) {
      const conductors = useTxn
        ? await Crew.find({ isArchived: false, status: 'Available', role: 'Conductor' }).session(session).lean()
        : await Crew.find({ isArchived: false, status: 'Available', role: 'Conductor' }).lean();
      ...
      const conBusy = new Set(conOverlaps.map(a => a.crewId.toString()));
      const freeConductors = conductors.filter(c => !conBusy.has(c._id.toString()))
        .filter(c => {
          const minRest = settings?.minRestHours ?? 12;
          if (c.lastDutyEnd) {
            const minStart = new Date(c.lastDutyEnd);
      ...
      for (const c of freeConductors) {
        const existingC = useTxn
          ? await Assignment.find({ crewId: c._id, status: { $in: ['Planned','Live'] }, ...overlapsQuery(start, end) }).session(session)
          : await Assignment.find({ crewId: c._id, status: { $in: ['Planned','Live'] }, ...overlapsQuery(start, end) });
        const conflicts = await evaluateConstraints({
          settings, crew: c, bus: chosen.bus, route,
          existingCrewAssignments: existingC, existingBusAssignments: [],
