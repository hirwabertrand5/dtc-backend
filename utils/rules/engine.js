...
async function evaluateConstraints({
  ...
  if (crew.status && crew.status !== 'Available') reasons.push(`Crew status is ${crew.status}`);
  ...
  if (freezeWindowHours && dayjs(startTime).diff(dayjs(), 'hour', true) < freezeWindowHours)
    reasons.push(`Inside freeze window (${freezeWindowHours}h)`);

  if (conductorRequired && role === 'Driver') reasons.push('Conductor required');
